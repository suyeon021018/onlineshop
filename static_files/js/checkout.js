$(function (){
    ...
    IMP.init('imp07772071'); //iamport 관리자 페이지에 있는 본인의 가맹점 식별코드(ex. imp00000000)를 입

    $('.order-form').on('submit', function (e) { //order_form에 submit 버튼을 눌렀을 때 function을 동작시
        var amount = parseFloat($('.order-form input[name="amount"]').val().replace(',', '')); // 결제 금
        ...
        if (merchant_id !== ''){ // !== : type까지 같은지 체크
            ...
            IMP.request_pay({ //결제 요청 부분, iamport 메뉴얼에서 정의해준 포맷
                pg: "html5_inicis", // KG이니시스(html5_inicis), 카카오페이(kakao), 나이스(nice)
                pay_method: "card",
...
function AjaxCreateOrder(e){
    ...
    var request = $.ajax({
        method: "POST",
        url: order_create_url, //요청이 전송될 url, order 생성 뷰 호출, templates(order\templates\order\
